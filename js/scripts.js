...
// add event for Click to Start Game button click.
...
function startGame() {
    ...
    // for the stars on the score, prints the number of attempts.
    updateScore();
}

function clearGrid() {
    // before clear all HTML elements in the game board,must first un-subscribe to any event listener that,we may have added earlier, to avoid causing performance issues
    // with repeated gameplay (yeas right, as if people will want to play this game repeatidly!)
    ...
    // now can safely clear all HTML elements from the game board
    document.getElementById("game-board").innerHTML = "";
}

function generateRandomCards() {
    // have eight icons, each icon has a duplicate match
    let icons = ["microphone",    "microphone",
                 "volume-up",     "volume-up",
...
function secondsToTimeFormat(secondsCounter) {
    ...
    // here can now construct the time from all of the above components
